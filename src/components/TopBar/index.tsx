...
import { fetchAdminInfo } from '../../stores/authActions';
...
function Main() {
  const [admin,SetAdmin]=useState(Intailstate)
  ...
  useEffect(() => {
   let currentAdmin= dispatch(fetchAdminInfo());
   currentAdmin.then((res:any)=>{
   
    SetAdmin(res?.payload)
   })
  }, [dispatch]);
  ...
  const Deconnecter = async () => {
    try {
      ...
      localStorage.removeItem("isConnecte");
        ...
        <Menu>
          ...
          <Menu.Items className="w-56 mt-px text-white bg-primary">
            ...
            <Menu.Item className="hover:bg-white/5" onClick={Deconnecter}>
              <Lucide icon="ToggleRight" className="w-4 h-4 mr-2" /> Deconnecter
            </Menu.Item>
          </Menu.Items>
        </Menu>
      </div>
